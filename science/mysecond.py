...
class Plotting:
	...
	@staticmethod
	def algo2(inp_img,j):
		labels = Finding.img_to_labels3(inp_img,j)
		out_fig = Plotting.make_windows(labels,inp_img)
		return out_fig
	
	@staticmethod
	def algo4(inp_img,j,display_img):
		labels = Finding.img_to_labels3(inp_img,j)
		out_fig = Plotting.make_windows2(labels,inp_img,display_img)
		return out_fig
	...
	@staticmethod
	def make_windows2(inp_labels, inp_img, display_img):
		bb0 = []
		for i in range(1,inp_labels.max() + 1):
			label_i = (inp_labels == i)
			bb0.append(Plotting.boundbox1(label_i))
		...
		fig = None
		s0 = None
		fig, (s0) = plt.subplots(nrows = 1, ncols = 1, figsize = (20,20))
		s0.imshow(display_img, cmap='gray')
		for rect in rect_arr:
			s0.add_patch(rect)
		return fig
		fig.show()
